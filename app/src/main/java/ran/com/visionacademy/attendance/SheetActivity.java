    ...
    private void showTable() {
        ...
        for (int i = 1; i < rowSize; i++) {
            ...
            for (int j = 1; j <= DAY_IN_MONTH; j++) {
                ...
                String status = dbHelper.getStatus(idArray[i - 1], date); //01.02.2021
        ...
        for (int i = 0; i < rowSize; i++) {
            ...
            roll_tvs[i].setPadding(11, 11, 11, 11);
            name_tvs[i].setPadding(11, 11, 11, 11);

            rows[i].addView(roll_tvs[i]);
            rows[i].addView(name_tvs[i]);

            for (int j = 1; j <= DAY_IN_MONTH; j++) {
                status_tvs[i][j].setPadding(11, 11, 11, 11);
